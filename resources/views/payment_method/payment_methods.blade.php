...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="d-flex flex-wrap mb-4">
            <div class="mr-auto">
                <span class="text-title">{{ __("Payment Methods") }}</span>
            </div>
            <div class="">
                @if (check_access(array('A_ADD_PAYMENT_METHOD'), true))
                    <a href="{{ route('add_payment_method')}}" role="button" class="btn btn-primary">{{ __("New Payment Method") }}</a>
                @endif
            </div>
        </div>
...
@push('scripts')
    ...
    <script src="{{ secure_asset('js/pages/payment_methods.js') }}"></script>
    <script>
        'use strict';
