@extends('layouts.layout')

@section("content")
<div class="row">
    <div class="col-md-12">
        
        <div class="d-flex flex-wrap mb-4">
            <div class="mr-auto">
                <span class="text-title">{{ __("Billing Counters") }}</span>
            </div>
            <div class="">
                @if (check_access(array('A_ADD_BILLING_COUNTER'), true))
                    <a href="{{ route('add_billing_counter')}}" role="button" class="btn btn-primary">{{ __("New Billing Counter") }}</a>
                @endif
            </div>
        </div>

        <div class="table-responsive">
            <table id="listing-table" class="table display nowrap w-100">
                <thead>
                    <tr>
                        <th>{{ __("Counter Code") }}</th>
                        <th>{{ __("Counter Name") }}</th>
                        <th>{{ __("Status") }}</th>
                        <th>{{ __("Created On") }}</th>
                        <th>{{ __("Updated On") }}</th>
                        <th>{{ __("Created By") }}</th>
                        <th>{{ __("Action") }}</th>
                    </tr>
                </thead>
                <tbody>
                </tbody>
            </table>
        </div>

    </div>
</div>
@endsection

@push('scripts')
    <script src="{{ secure_asset('plugins/datatables/jquery.dataTables.min.js') }}"></script>
    <script src="{{ secure_asset('plugins/datatables/dataTables.bootstrap4.min.js') }}"></script>
    <script src="{{ secure_asset('js/datatable.js') }}"></script>
    <script src="{{ secure_asset('js/pages/billing_counters.js') }}"></script>
    <script>
        'use strict';
        var billing_counter = new BillingCounter();
        billing_counter.load_listing_table();
    </script>
@endpush