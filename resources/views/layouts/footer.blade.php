...
<footer class="container-fluid p-3 {{ (isset($fixed_footer) && $fixed_footer) ? "fixed-bottom" : "" }}">
    <div class="d-flex justify-content-between">
        <span>&nbsp;© {{ date("Y") }}-{{ date("Y", strtotime("+1 year")) }} {{ config('app.company') }} &middot; <span class="text-muted">v5.1</span></span>
        <languageswitchercomponent :languages="{{ json_encode($languages) }}" :selected_language="{{ json_encode($selected_language) }}"></languageswitchercomponent>
    </div>
</footer>